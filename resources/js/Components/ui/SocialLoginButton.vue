...
interface Props {
  provider: 'google' | 'github' | 'linkedin' | string
  disabled?: boolean
  customText?: string
}
...
const socialUrl = computed(() => {
  return route('auth.social.redirect', { provider: props.provider })
})
...
const buttonText = computed(() => {
  ...
  const providerName = props.provider.charAt(0).toUpperCase() + props.provider.slice(1)
