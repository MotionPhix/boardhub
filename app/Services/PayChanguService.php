...
class PayChanguService
{
    ...
    private function processMobileMoneyPayment(Payment $payment, array $paymentData): array
    {
        $payload = [
            ...
            'callback_url' => route('api.payments.webhook', ['provider' => 'paychangu']),
            'meta' => [
    ...
    public function checkPaymentStatus(Payment $payment): array
    {
        try {
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $this->secretKey,
                'Accept' => 'application/json'
            ])->get($this->baseUrl . '/verify-payment/' . $payment->external_id);
            ...
            if ($payment->status !== $mappedStatus) {
                ...
                if ($status === 'success') {
                    ...
                    PaymentFailed::fire(
                        ...
                        provider: $payment->provider,
                        amount: $payment->amount,
                        failure_reason: $responseData['data']['logs'][0]['message'] ?? 'Payment failed'
                    );
                }
            }
